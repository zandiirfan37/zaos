...
| `.agents/skills/` | LIVE | Sole shared-skill root. On-demand only. TESTED: `ui-ux-pro-max` (DOMAIN), `ask-the-council` (REASONING), `ml-research` (DOMAIN), `browser-qa` (WORKFLOW), `systematic-debugging`, `scope-discipline`, and `code-review` (fast-intake procedural capabilities). PATCH: `lit-review` (real pilot blocked by available synthesizer auth; not trusted). DRAFT: `project-refoundation`, `project-architecture`, `medical-imaging-research`. Skill files follow the Agent Skills shape (`SKILL.md` + `references/`); `ZANDI_SKILL.md` records provenance. |
